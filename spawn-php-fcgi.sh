...
if [ ! $? -eq 0 ]; then
	...
	PHP_U="$PHP -c /services/etc/php.ini"
	...
	env -i $FCGI_E $SPAWNFCGI -s $SOCKET -f "$PHP_U" -u starkast -g starkast -c $CHROOT -C $PHP_FCGI_CHILDREN -P $SOCKETDIR/php.pid 
...
for i in ${USERS}; do
	...
	env -i $FCGI_E $SPAWNFCGI -s $SOCKET -f "$PHP_U" -u $i -g $i -c $CHROOT -C $PHP_FCGI_CHILDREN -P $SOCKETDIR.pid 
	chmod -R 770 $SOCKETDIR
